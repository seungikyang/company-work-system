...
각 문제는 PRD/TRD 문서의 어떤 절을 구현하는지 함께 적었습니다. 이 문서는 암기 시험지가 아니라 실행 프로젝트를 만들기 전 설계 판단과 핵심 코드를 연습하는 워크시트입니다.

## 문제 사용 규칙

1. [HTML 취업 워크북](../index.html)에서 오늘 구현할 FR ID와 챕터를 고릅니다.
2. 선택 패널의 취업형 문제에 핵심 책임·실패 조건·검증 방법을 먼저 적습니다.
3. `hard/` 또는 `starter/`에서 직접 작성하고, 막히는 만큼만 HTML의 힌트를 확인합니다.
4. 실제 오류를 기록한 뒤 정답 기준과 `answers.md`를 확인해 답을 보완합니다.
5. 같은 기능을 실행 가능한 Spring Boot 프로젝트에 구현합니다.
6. 테스트 결과와 코드·커밋 링크를 [취업 준비 통합 워크북](./job-preparation-workbook.md)에 남깁니다.

문제의 빈칸을 모두 채워도 실행 코드와 검증 증거가 없으면 해당 챕터는 완료가 아닙니다.

## 문제 풀이 기록 카드

문제를 풀기 전에 오늘 만들 증거를 먼저 정합니다.
...
## 2. Department 엔티티 (TRD 3.3.3, 3.5.2)

파일: `starter/02-department-entity/Department.entity.java`

- 부서명(`name`)을 unique 로 잡는 이유를 답한다.
- Department 에서 Employee 방향 연관관계를 매핑할지(`@OneToMany(mappedBy=...)`) 또는 단방향으로 둘지 트레이드오프를 적어본다.
- DB 컬럼 길이(VARCHAR(100), VARCHAR(255)) 와 도메인 의미를 매칭한다.

## 3. Employee 엔티티 (TRD 3.3.2, 3.5.3, 3.6.2)

파일: `starter/03-employee-entity/Employee.entity.java`

- `User` 와 1:1, `Department` 와 N:1 관계를 매핑한다.
- `fetch = LAZY` 가 기본이 되어야 하는 이유를 답한다.
- `EmployeeStatus` (ACTIVE / INACTIVE / RESIGNED) enum 을 작성한다.
- 퇴사 처리 시 데이터를 DELETE 하지 않고 `RESIGNED` 로 두는 이유를 답한다.

## 4. LeaveRequest 엔티티와 enum (TRD 3.3.4, 3.6.3, 3.6.4)

파일: `starter/04-leave-entity/LeaveRequest.entity.java`

- 휴가 시작일/종료일을 `LocalDate` 로 저장하는 이유.
- `status` 의 초기값을 `PENDING` 으로 두는 정적 팩토리 메서드를 작성한다.
- 승인 시 `approverId`, 반려 시 `rejectReason` 이 들어가는 자리를 채운다.
- 신청자(`employeeId`)와 승인자(`approverId`)가 서로 다를 수 있도록 유지한다.

## 5. Notice 엔티티 (TRD 3.3.5, 3.5.5)
...
## 6. ApprovalDocument 엔티티 (TRD 3.3.6, 3.6.4)

파일: `starter/06-approval-entity/ApprovalDocument.entity.java`

- `ApprovalStatus` enum 의 상태 전이 그래프를 그려본다. (`DRAFT → PENDING → APPROVED/REJECTED`)
- 작성자(`writerId`)와 결재자(`approverId`)가 같으면 안 되는 도메인 규칙을 어디에서 검증할지 정한다.
- `approvedAt` 컬럼이 nullable 인 이유를 답한다.

## 7. Repository 메서드 쿼리 (TRD 3.7.2, 3.8.1)

파일: `starter/07-repository/EmployeeRepository.java`

- `existsByEmail`, `findByEmail` 같은 메서드 이름 쿼리의 규칙.
- 검색을 위한 `findByNameContainingOrEmailContaining` 시그니처 채우기.
- `Pageable` 을 사용하는 페이징 메서드 시그니처 채우기.
- 직원이 어떤 부서에 속한 사람만 보고 싶을 때 `findByDepartmentId` 같은 쿼리를 추가한다.

## 8. 직원 등록 Service (TRD 3.10.1)

파일: `starter/08-employee-register/EmployeeService.register.java`

- `@Transactional` 의 위치와 의미.
- 이메일/사번 중복 검사를 어디서 하는지.
- User 와 Employee 가 함께 생성되어야 하는 이유.
- 트랜잭션 도중 예외가 발생하면 어떤 일이 일어나는지.

## 9. 직원 검색 + 페이징 (TRD 3.5.3, 2.5.2)

파일: `starter/09-employee-search/EmployeeService.search.java`

- `Pageable` 을 어떻게 받고 어떻게 넘기는지.
- `Page<Employee>` 를 `Page<EmployeeResponse>` 로 변환하는 방법.
- 정렬 기준을 클라이언트에서 받을지, 서버에서 고정할지 결정한다.

## 10. 휴가 신청 Service (TRD 3.8.2)

파일: `starter/10-leave-request/LeaveService.request.java`

- 시작일이 종료일보다 늦으면 어떤 ErrorCode 로 막을지.
- 퇴사 상태의 직원이 신청하지 못하게 막는 규칙.
- 신청 시 `status` 기본값을 어디서 세팅할지.

## 11. 휴가 승인/반려 Service (TRD 3.8.2, 3.10.2)

파일: `starter/11-leave-approval/LeaveService.approval.java`

- `PENDING` 상태가 아닌 휴가에 대해 승인/반려를 막는 검증 위치.
- 반려 시 `rejectReason` 이 비어 있으면 어떻게 처리할지.
- 승인 시 `approverId` 를 채우는 시점.
- 동시에 두 관리자가 같은 휴가를 승인하면 어떻게 되는지 생각해본다.

## 12. 공지사항 Service (TRD 3.8.3)
...
## 13. 결재 문서 작성/요청 Service (TRD 3.8.4)

파일: `starter/13-approval-document/ApprovalService.document.java`

- 작성과 결재 요청을 같은 API 로 둘지, 분리(`POST` / `PATCH /submit`)할지의 트레이드오프.
- `DRAFT → PENDING` 으로 상태 변경 시 검증할 항목.
- 작성자와 결재자가 같을 수 없도록 막는 검증 위치.

## 14. 결재 승인/반려 Service (TRD 3.8.4, 3.10.3)

파일: `starter/14-approval-decision/ApprovalService.decision.java`

- `PENDING` 상태가 아닌 문서에 대한 결재 시도를 막는다.
- 결재자가 본인이 맞는지 확인한다.
- 승인 시 `approvedAt` 을 세팅한다.
- 반려 시 사유 필수.
...
- 도메인별 에러 코드(`EMPLOYEE_NOT_FOUND`, `INVALID_DATE_RANGE` 등)를 enum 으로 모은다.
...
파일: `starter/17-dto-validation/EmployeeCreateRequest.java`

- `@NotBlank`, `@Email`, `@Size`, `@Pattern` 의 차이.
- `@Valid` 가 어떤 계층에서 동작하는지.
- 응답 DTO 와 요청 DTO 를 분리하는 이유.

## 18. EmployeeController (TRD 3.7.2)

파일: `starter/18-controller-employee/EmployeeController.java`

- 경로 설계(`/api/employees`)와 HTTP 메서드 매칭.
- `@PathVariable`, `@RequestBody`, `@RequestParam` 의 차이.
- 관리자만 접근하도록 권한 검증을 어디서 하는지.

## 19. LeaveController (TRD 3.7.4)

파일: `starter/19-controller-leave/LeaveController.java`

- `/api/leaves` (USER) 와 `/api/admin/leaves` (ADMIN) 경로 분리.
- `PATCH /api/admin/leaves/{leaveId}/approve` 같은 동사형 경로의 장단점.
...
- 부서명 중복 검증 (`existsByName`) 과 DB unique 의 다층 방어.
- 부서 삭제 시 소속 직원이 있을 때의 정책(거부 / 기본 부서 이동 / NULL 허용).
- 부서 상세에서 소속 직원 목록까지 N+1 없이 조회하는 방법.
- 부서 목록을 페이징 없이 전체 조회로 둔 이유와, 부서가 1000개를 넘었을 때의 대안.

## 25. 부서 Controller + DTO (TRD 3.7.3)

파일: `starter/25-department-controller/DepartmentController.java`

- `@PreAuthorize` 와 Service 가드의 다층 권한 검사.
- 전체 갱신(PUT) vs 부분 갱신(PATCH) 선택.
- `DepartmentDetailResponse` 가 소속 직원까지 포함하는 nested record 설계.

## 26. 공지 Controller + DTO (TRD 3.7.5)
...
## 27. 결재 Controller + DTO — my / pending / submit / approve / reject (TRD 3.7.6)

파일: `starter/27-approval-controller/ApprovalController.java`

- `/api/approvals/my` (USER) 와 `/api/approvals/pending` (APPROVER/ADMIN) 의 권한 차이.
...
- 부서 변경 시 부서 존재 확인 + 도메인 메서드(`changeDepartment`) 사용.
...
## 31. Repository — User / Department / Leave / Notice / Approval (TRD 3.5)

파일: `starter/31-repositories-all/AllRepositories.java`

- `findByUser_Email` 같은 nested property 메서드 쿼리.
- 휴가 기간 겹침 검사 JPQL (`startDate <= :endDate AND endDate >= :startDate`).
...
- `EmployeeResponse.from(employee)` 같은 정적 팩토리.
- `Page.map(EmployeeResponse::from)` 으로 페이지 메타 정보 유지.
...
- 휴가 승인 중복 처리 / 직원 등록 부분 저장 / Entity 직접 반환 / LazyInitializationException / 이메일 동시 가입 race.

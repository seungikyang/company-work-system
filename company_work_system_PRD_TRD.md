...
> 문서 목적: SI 취업용 포트폴리오 프로젝트인 **사내 업무관리 시스템(Internal Work Management System)** 의 제품 요구사항(PRD)과 기술 요구사항(TRD)을 정리한다.  
> 프로젝트 유형: Java Spring Boot 기반 업무 시스템  
...
이 문서는 사내 업무관리 시스템을 개발하기 전에 필요한 기능 요구사항과 기술 설계 내용을 정리하기 위한 문서다.
...
이 프로젝트는 신입 개발자가 SI 업무 흐름을 이해하고, 실무형 포트폴리오를 만들기 위해 기획한 사내 업무관리 시스템이다.
...
사내 업무관리 시스템은 SI 취업 포트폴리오로 적합한 프로젝트다.
...
이 문서를 기준으로 구현하면 단순 예제 프로젝트가 아니라, SI 취업 면접에서 설명 가능한 업무 시스템 포트폴리오로 발전시킬 수 있다.
